...
6. 只输出整理结果，不要输出分析过程。`,
  ...
  {
    id: 'priceScheduleFormat', label: '报价表格式', required: false, output: 'json', description: '招标文件规定的分项报价表固定格式列定义。',
    prompt: () => `任务：提取招标文件、询比文件或采购文件中规定的投标报价表固定格式。
...
2. columns 数组严格按原文表格的列顺序输出；column_name 逐字保留原文列头文字（原文含 * 号、括号、空格等符号时原样保留），不要增删列、不要改写列名。
3. is_required 仅作记录：原文以 * 号、“必填”等标识的填“是”，否则填“否”；fill_instruction 填这一列要求填写什么内容。
