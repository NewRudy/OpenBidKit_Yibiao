...
function createChildrenPrompt({ hasOriginalPlan, hasCustomOutline, originalOnly, targetLeafCount, allowRootChanges }) {
...
async function runOutlineGenerationTaskV2({ agentService, workspaceStore, knowledgeBaseService, updateTask, checkpointTask, taskControl, payload }) {
  ...
  function continueWithChildrenGeneration(allocations) {
    ...
    return {
      ...
      prompt: createChildrenPrompt({ hasOriginalPlan, hasCustomOutline, originalOnly, targetLeafCount, allowRootChanges }),
      files: [
