...
请完成技术评分项结构化和目录规划：
1. 阅读 ${OUTLINE_OUTPUT_FILE}、技术评分信息.md，以及存在的原方案.md、自有大纲.md 和参考知识库目录。
...
function createChildrenPrompt({ hasOriginalPlan, hasCustomOutline, originalOnly, targetLeafCount, allowRootChanges, standaloneTechnical }) {
  const branchInstruction = hasCustomOutline
    ? '已上传自有大纲：以自有大纲.md 为主建立规划层级及以下目录，再用技术评分信息.md 补充自有大纲语义上确实缺失的技术要求，意思相近的内容不要重复添加。'
    : !hasOriginalPlan
...
9. 如果存在参考知识库、原方案或自有大纲，只能用于完善评分项对应节点的下级结构，不得改变评分项映射或引入未经批准的同层级大项。
...
async function runOutlineGenerationTaskV2({ aiService, agentService, ordinaryAgentService, workspaceStore, knowledgeBaseService, openXmlHelperService, updateTask, checkpointTask, taskControl, payload }) {
  ...
  const customOutlineRequested = payload?.custom_outline_mode === 'custom-complement' && storedPlan.workflowKind !== 'existing-plan-expansion';
  if (customOutlineRequested && !storedPlan.customOutlineFile) {
    throw new Error('请先上传自有大纲，再生成目录');
  }
  const customOutlineMarkdown = customOutlineRequested ? String(workspaceStore.readCustomOutlineMarkdown?.() || '') : '';
  const hasCustomOutline = customOutlineRequested && customOutlineMarkdown.trim().length > 0;
  if (customOutlineRequested && !hasCustomOutline) {
    throw new Error('请先上传自有大纲，再生成目录');
  }
  ...
  if (originalOnly) {
    ...
    initialFiles = [
      ...
      ...(hasCustomOutline ? [{ path: '自有大纲.md', content: customOutlineMarkdown }] : []),
    ];
    taskInstruction = standaloneTechnical
      ...
      : hasOriginalPlan
        ...
        : hasCustomOutline
          ? '优先按照自有大纲.md 提取和组织一级目录，它是目录结构和标题来源的主要依据；响应文件要求.md 仅用于补充自有大纲语义上确实缺失的内容，意思相近的内容不要重复添加。'
      ...
      stats: {
        ...
        agent: {
          ...
          resume_payload: {
            ...
            ...(customOutlineRequested ? { custom_outline_mode: 'custom-complement' } : {}),
  ...
  function continueWithChildrenGeneration(allocations) {
    ...
    return {
      ...
      prompt: createChildrenPrompt({ hasOriginalPlan, hasCustomOutline, originalOnly, targetLeafCount, allowRootChanges, standaloneTechnical }),
      files: [
