...
import type { BackgroundTaskState, OutlineSelectionItem, SaveOutlineRequest, SaveOutlineSelectionRequest, TechnicalPlanCustomOutlineFile, TechnicalPlanState, TechnicalPlanWorkflowKind } from '../types';
...
const customOutlineModeOptions: Array<{ value: CustomOutlineGenerationMode; title: string; description: string; requiresFile: boolean }> = [
  ...
  },
  {
    ...
    title: '自有大纲优先',
    description: '保留上传大纲的一级目录骨架，再补齐招标评分项和下级响应目录。',
    requiresFile: true,
  },
];
...
function OutlineEditPage({
  ...
  }, [generationDialogOpen, isExpansionWorkflow, outlineExpansionMode, outlineWordControlOptions, customOutlineFile, referenceKnowledgeDocumentIds]);
  ...
  const importCustomOutlineDocument = async () => {
    if (generating || importingCustomOutline) return;
    try {
      ...
      if (!result.state) throw new Error('自有大纲解析结果为空');
  ...
  const generateOutline = async () => {
    ...
    try {
      ...
      await window.yibiao?.tasks.startOutlineGeneration({
        ...
        custom_outline_mode: isExpansionWorkflow ? 'none' : draftCustomOutlineMode,
        word_control_options: wordControlOptions,
      });
      trackConfigUsage({
        outline_mode: isExpansionWorkflow ? nextOutlineExpansionMode : draftCustomOutlineMode === 'custom-complement' ? 'custom-outline' : nextOutlineMode,
  ...
  const cancelOutlineGeneration = async () => {
    if (!taskRunning || cancelingOutline) return;
    try {
  ...
  const renderCustomOutlinePicker = () => {
    if (isExpansionWorkflow) return null;
    return (
        ...
        <div className="custom-outline-upload-card">
          {customOutlineFile ? (
            ...
            <div className="custom-outline-empty">
              ...
              <span>可上传 Word、PDF 或 Markdown 大纲草稿，作为目录生成骨架。</span>
            </div>
          )}
          <button type="button" className="secondary-action" onClick={() => { void importCustomOutlineDocument(); }} disabled={generating || importingCustomOutline || contentMutationLocked}>
          ...
          {customOutlineModeOptions.map((option) => {
            ...
            return (
              <button
                ...
                disabled={generating || (option.requiresFile && !customOutlineFile)}
                aria-pressed={selected}
              >
                <strong>{option.title}</strong>
                <span>{option.description}</span>
    ...
    <div className="plan-step-body outline-generation-page">
      <section className="outline-command-bar">
        <div>
          ...
          <strong>目录生成</strong>
          <p>{isExpansionWorkflow ? `当前原方案目录使用方式：${outlineExpansionModeLabels[outlineExpansionMode]}；参考知识库：${referenceKnowledgeDocumentIds.length ? `已选择 ${referenceKnowledgeDocumentIds.length} 个文档` : '未选择'}。` : `自有大纲：${customOutlineFile ? customOutlineFile.fileName : '未上传'}；参考知识库：${referenceKnowledgeDocumentIds.length ? `已选择 ${referenceKnowledgeDocumentIds.length} 个文档` : '未选择'}。`}</p>
        </div>
        <div className="outline-command-actions">
          ...
          {taskRunning && (
            <button type="button" className="danger-action" onClick={() => { void cancelOutlineGeneration(); }} disabled={cancelingOutline}>
